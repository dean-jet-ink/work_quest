...
export const ProfileFormModal = memo((props: Props) => {
  const { user, onClose, isOpen, onChange, src } = props;
  const { initialValues, onSubmit, validationSchema } = useProfile(user);
          ...
          {({ isSubmitting, handleSubmit, setFieldValue }) => (
            <Form
              onSubmit={(e) => {
                e.preventDefault();
                handleSubmit();
                setFieldValue("picture", src);
              }}
            >
              <Stack spacing={3}>
                <PrimaryInputFile name="file" src={src} onChange={onChange} />
